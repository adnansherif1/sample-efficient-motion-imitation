...
def inv_kin(root_orient, model_coords, pose_coords, parents, terminal):
    ...
    for i in range(1, N):
        if i in terminal: # the rotations of terminal joints don't actually matter b/c they have no children
            ...
            # R1 = get_R(model_coords[children] - model_coords[i]) 
            # R2 = get_R((np.linalg.inv(As[parents[i]])@(pose_coords[children] - pose_coords[i]).T).T )
            # R_here = R2@R1.T
...
# for joint_dict in joints:
# 	if joint_dict['Parent'] == -1:
# 		ax.scatter(joint_coords[i,0], joint_coords[i, 1], joint_coords[i, 2])
# 	else: 
# 		ax.plot(joint_coords[[i, joint_dict['Parent']], 0], joint_coords[[i, joint_dict['Parent']], 1], joint_coords[[i, joint_dict['Parent']], 2], c='blue')

# 	i+=1

# # ax.scatter(smpl_joints[0, :, 0], smpl_joints[0, :, 2], smpl_joints[0, :,1], c='green')

# for i in range(len(kt[0])):
# 	if kt[0][i] > 50 :
# 		ax.scatter(smpl_joints[0, kt[1][i], 0]+1, smpl_joints[0, kt[1][i], 1], smpl_joints[0, kt[1][i],2])
# 	else: 
# 		ax.plot(smpl_joints[0, [kt[0][i], kt[1][i]], 0].flatten()+1, smpl_joints[0, [kt[0][i], kt[1][i]], 1].flatten(), smpl_joints[0, [kt[0][i], kt[1][i]],2].flatten(), c='green')
...
print(corrected[0, 0] - smpl_joints[0, 0])
print(smpl_joints[0, 0])
print(joint_coords[0])
corrected[:, 0] = (corrected[:, 1] + corrected[:, 2])/2
# root_delta = joint_coords[0] - (smpl_joints[:, 1] + smpl_joints[:, 2])/2 
# corrected += root_delta.reshape(-1,1,3)
...
# for t in range(smpl_joints.shape[0]):
# 	Rs = inv_kin(hmmr_data['poses'][t,0], hmmr_data['J'][t], hmmr_data['J_transformed'][t], list(kt[0]), [10, 11, 15, 22, 23])
# 	c, _ = batch_global_rigid_transformation(Rs, np.array([hmmr_data['J'][t]]), kt[0])
# 	# print(np.concatenate([np.arange(24).reshape(-1,1), np.abs(c[0]-J_ps)], axis=1))
# 	assert np.sum(np.abs(c[0]-hmmr_data['J_transformed'][t])) < 1e-5, "large error on frame %d" % t
# 	for i in range(len(Rs[0])):
# 		R = Rs[0, i]
# 		try:
# 			quaternion_from_matrix(R)
# 		except ValueError:
# 			print("Non-invertible or non-unit rotation matrix at joint %d" %  i)
...
# scatter2 = ax.scatter(corrected[0, :, 0], corrected[0, :, 1], corrected[0,:, 2], c='blue')
...
# anim = animation.FuncAnimation(fig, update_scatter, posed_smpl.shape[0], fargs=([posed_smpl, scatter, corrected, scatter2]),
...
tiled_joint_coords = np.tile(joint_coords.reshape(1, 15, 3), (T, 1, 1))
...
# start_frame = joint_coords_retargeting[0] + (corrected[0, 0] - joint_coords_retargeting[0, 0])

# to_plot = ((initial_rot_offset@hmmr_data['poses'][0,0])@start_frame.T).T
# ax.scatter(to_plot[:, 0], to_plot[:, 1], to_plot[:, 2], c='blue')
# ax.scatter(corrected[0, :, 0], corrected[0, :, 1], corrected[0, :, 2], c='green')
		...
		if i == 0:
			...
			try:
				...
				if i in one_d_rotations:
					# quart.append(np.array([q[0]]))

					end_orient = np.linalg.inv(retargeted_As[0, i-1])@(corrected[t, i+1] - corrected[t, i])
					start_orient = (start_frame[i+1] - start_frame[i])
					# print(retargeted_Rs[0, i]@start_orient)
					# print(end_orient)
					# print()
					if np.all(end_orient == -start_orient):
						theta = np.pi
					else:
						# temp_orient = rotation_matrix_from_vectors(start_orient, np.array([1,0,0]))
						# print(temp_orient@start_orient)
						# temp_start = temp_orient@start_orient
						# temp_end = temp_orient@end_orient
						# print(temp_end)
